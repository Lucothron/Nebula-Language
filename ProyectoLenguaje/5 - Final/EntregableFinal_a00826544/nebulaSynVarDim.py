...
def p_VECDEF(p):
    ...
    m = x
    if p[1] not in variables.keys():
        variables[p[1]] = [p[6], 'vec', x, m, dimCont]
    else:
        print("\tVARIABLE ALREADY EXISTS")
    opers.append('vec')
    dimQuad(p[3], 1, 1)
...
def p_MATDEF(p):
    ...
    m1 = y
    if p[1] not in variables.keys():
        variables[p[1]] = [p[8], 'mat', x, y, m, m1, dimCont]
    else:
        print("\tVARIABLE ALREADY EXISTS")
    opers.append('mat')
    dimQuad(p[3], p[5], 1)
...
def p_CUBEDEF(p):
    ...
    m2 = z
    if p[1] not in variables.keys():
        variables[p[1]] = [p[10], 'cube', x, y, z, m, m1, m2, dimCont]
    else:
        print("\tVARIABLE ALREADY EXISTS")
    opers.append('cube')
    dimQuad(p[3], p[5], p[7])
